...
# 캐릭터 관계
relationships = {
    "이현성": "피해자의 조카이자 유일한 유산 상속 대상자",
    "정 변호사": "피해자의 법률 자문을 맡던 변호사",
    "민 교수": "피해자의 학문적 라이벌",
    "혜 형사": "사건을 담당한 수사 형사"
}
...
# 증거
evidences = {
    "정 변호사": "지문: 정 변호사의 지문이 연구실 금고 손잡이에서 검출됨.",
    "이현성": "금융거래: 이현성이 교수의 사망 전 거액의 대출을 신청한 내역이 있음.",
    "민 교수": "노트북: 민 교수가 교수의 원본 연구 자료가 담긴 노트북을 무단 보관하고 있었음.",
    "혜 형사": "압력편지: 혜 형사가 받은 '사건을 더 이상 파지 말라'는 청와대발 익명 편지."
}

# 힌트 (섞어서 중복 방지)
hints = random.sample([
    "정 변호사는 연구 공개를 반대했고, 유언장 내용을 제일 먼저 열람했다.",
    "이현성은 최근 투자 실패로 거액을 잃고 있었으며, 상속을 노릴 이유가 충분했다.",
    "민 교수는 최 교수의 업적을 자기 것이라 주장해왔다.",
    "혜 형사는 상부의 외압을 여러 차례 폭로하려다 수사를 묵살당했다.",
    "유언장이 일부 조작된 흔적이 발견되었고, 정 변호사만 접근 가능했다.",
    "사건 당시, 연구실에 남겨진 발자국이 정장 구두였다는 제보가 있었다.",
    "노트북에서 삭제된 파일이 복구됐고, 암호화된 정치적 메일이 있었다.",
    "이현성은 알리바이가 확실하지 않다. 사망 시각에 연락 두절 상태였다.",
    "혜 형사는 피의자가 아닌 내부 고발자일 가능성이 높아 보인다.",
...
def reveal_result(guess):
    ...
    if guess == "정 변호사":
        st.balloons()
        st.success("✅ 정답입니다! 정 변호사가 진범입니다.")
    else:
        st.error("❌ 오답입니다. 진범은 '정 변호사'였습니다.")
    ...
    정 변호사는 고위층의 외압을 이용해 유언장을 조작하고, 연구를 은폐하려 했다.  
    이현성, 민 교수, 혜 형사 등은 각자의 동기를 가졌으나, 살인과는 직접 관련이 없던 것으로 밝혀졌다.
